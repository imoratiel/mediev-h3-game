...
class DivisionService {
        ...
        try {
            ...
            if (!hasFortress) {
                return res.status(400).json({
                    success: false,
                    message: 'Este territorio necesita una Fortaleza para proclamar un Pagus'
                });
            }
    ...
    async ProclaimDivision(req, res) {
        ...
        try {
            ...
            if (fiefs.length < minRequired) {
                ...
                return res.status(400).json({
                    success: false,
                    message: `Un Pagus requiere al menos ${minRequired} territorios (enviados: ${fiefs.length})`
                });
            }
            ...
            if (fiefs.length > maxLimit) {
                ...
                return res.status(400).json({
                    success: false,
                    message: `Un Pagus no puede tener más de ${maxLimit} territorios`
                });
            }
    ...
    async UpdateDivisionTax(req, res) {
        ...
        if (isNaN(divisionId)) {
            return res.status(400).json({ success: false, message: 'ID de Pagus inválido' });
        }
        ...
        try {
            ...
            if (!updated) {
                return res.status(404).json({ success: false, message: 'Pagus no encontrado o no te pertenece' });
            }
