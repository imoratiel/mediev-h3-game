    ...
    <div class="admin-panel">
      ...
      <div class="admin-header">
        <div class="admin-header-left">
          ...
          <div v-if="status" class="admin-header-meta">
            Turno <strong>{{ status.game.currentTurn }}</strong>
            <span class="meta-sep">·</span>
            {{ nextTurnLabel }}
            <span v-if="gamePaused"    class="hbadge hbadge-yellow">PAUSADO</span>
            <span v-if="engineRunning" class="hbadge hbadge-green">MOTOR ACTIVO</span>
            <span v-else               class="hbadge hbadge-red">MOTOR DETENIDO</span>
          </div>
        </div>
        <div class="admin-header-right">
          <span class="refresh-label">Auto 10s</span>
          <button class="icon-btn" @click="fetchStatus" :disabled="loading" title="Actualizar">{{ loading ? '⏳' : '🔄' }}</button>
          <button class="icon-btn close-btn" @click="$emit('close')" title="Cerrar">✕</button>
        </div>
      </div>
      ...
      <!-- Body: 3-column grid -->
      <div v-if="status" class="admin-body">

        <!-- ══ IZQUIERDA: CONTROLES + AGENTES IA ══ -->
        <div class="admin-col col-left">
          ...
          <div class="admin-card">
            ...
            <div class="process-list">
              <div v-for="proc in status.processes" :key="proc.id" class="process-row">
            ...
            <div v-if="engineProc?.startTime" class="uptime-row">
              ⏱️ Activo desde hace <strong>{{ formatUptime(engineProc.uptimeMs) }}</strong>
            </div>
          </div>
          ...
          <div class="admin-card">
            ...
            <div class="timing-grid">
              ...
              <div class="timing-item">
                ...
                <span class="timing-value" :class="{ 'next-turn-soon': nextTurnSoon }">{{ nextTurnLabel }}</span>
              </div>
            </div>
          </div>
          ...
          <!-- Control del Motor -->
          <div class="admin-card">
            <h3 class="card-title">🔧 Control del Motor</h3>
            <div class="controls-row">
              <button class="ctrl-btn btn-green" :disabled="engineRunning || acting" @click="handleStart">▶ Arrancar</button>
              <button class="ctrl-btn btn-red"   :disabled="!engineRunning || acting" @click="handleStop">■ Detener</button>
            </div>
            <div class="controls-row" style="margin-top:8px">
              <button class="ctrl-btn btn-orange" :disabled="gamePaused || acting"  @click="handlePause">⏸ Pausar</button>
              <button class="ctrl-btn btn-blue"   :disabled="!gamePaused || acting" @click="handleResume">▷ Reanudar</button>
            </div>
          </div>

          <!-- Forzar Procesos -->
          <div class="admin-card">
            <h3 class="card-title">⚡ Forzar Procesos</h3>
            <p class="card-hint">Ejecuta inmediatamente, sin esperar el turno automático.</p>
            <div class="controls-row">
              <button class="ctrl-btn btn-force" :disabled="acting" @click="handleForceTurn">⏭ Turno</button>
              <button class="ctrl-btn btn-force" :disabled="acting" @click="handleForceHarvest">🌾 Cosecha</button>
              <button class="ctrl-btn btn-force" :disabled="acting" @click="handleForceExploration">🔍 Explor.</button>
            </div>
          </div>

          <!-- Testing -->
          <div class="admin-card">
            <h3 class="card-title">🏛️ Testing</h3>
            <p class="card-hint">Crea una Comarca completa adyacente a tu territorio.</p>
            <div class="controls-row">
              ...
              <button class="ctrl-btn btn-force" :disabled="spawningDummy" @click="handleSpawnDummy">
                {{ spawningDummy ? '⏳ Invocando...' : '🪆 DUMMY' }}
              </button>
            </div>
            <p v-if="pagusMsg" class="card-hint" :style="{ color: pagusOk ? '#86efac' : '#f87171', marginTop:'6px' }">
              {{ pagusOk ? '✅' : '❌' }} {{ pagusMsg }}
            </p>
          </div>
          ...
          <div class="admin-card danger-card">
            ...
            <p class="card-hint danger-hint">Estas acciones son irreversibles.</p>
            ...
            <div v-else class="reset-confirm-box">
              ...
              <div class="reset-confirm-actions">
                ...
                <button class="ctrl-btn btn-cancel" @click="showResetConfirm = false" :disabled="acting">Cancelar</button>
              </div>
            </div>
          </div>

          <!-- Agentes IA -->
          <div class="admin-card fill-card">
            <h3 class="card-title">
              🤖 Agentes IA
            ...
            <div class="ai-control-strip">
              <button class="ai-toggle-btn" :class="aiEnabled ? 'toggle-on' : 'toggle-off'"
                :disabled="settingsLoading" @click="handleToggleAI">
                <span class="toggle-dot"></span>
                <span class="toggle-label">{{ aiEnabled ? 'IA Activa' : 'IA Off' }}</span>
              </button>
              <select v-model="aiProvider" class="ai-spawn-select" :disabled="settingsLoading"
              ...
              <input v-model.number="aiBudget" type="number" min="1000" step="10000"
                class="ai-budget-input" :disabled="settingsLoading" title="Budget en tokens" />
              <button class="ctrl-btn btn-ai-save" :disabled="settingsLoading"
                @click="handleSaveSetting('max_token_budget', aiBudget)">💾</button>
              <button class="ctrl-btn btn-ai-test"
                :disabled="settingsLoading || testing || aiProvider === 'procedural'"
                @click="handleTestConnection">{{ testing ? '⏳' : '🔌 Probar' }}</button>
            </div>
            ...
            <div class="ai-spawn-row">
              ...
              <input v-model.number="spawnCount" type="number" min="1" max="10"
                class="ai-spawn-input" :disabled="spawning"
              ...
              <button class="ctrl-btn btn-ai-spawn" :disabled="spawning || acting" @click="handleSpawnAgents">
                {{ spawning ? '⏳' : '＋ Crear' }}
              </button>
              <button class="ctrl-btn btn-ai-cycle" :disabled="acting || spawning" @click="handleForceAITurn">
                🔄 Ciclo IA
              </button>
            </div>
              ...
              <table class="ai-table">
                <thead>
                  <tr>
                    ...
                    <th class="ai-th ai-th-num">🏰 Territ.</th>
                ...
                <tbody>
                  <tr v-for="agent in agents" :key="agent.player_id" class="ai-row">
                    <td class="ai-td ai-td-name">
                      <span class="ai-color-dot" :style="{ background: agent.color || '#888' }"></span>
                      {{ agent.display_name }}
                    </td>
                    <td class="ai-td ai-td-center">
                      <span class="ai-profile-badge" :class="{
                    ...
                    <td class="ai-td ai-td-act">
                      <button v-if="agent.capital_h3" class="ai-goto-btn"
                        @click="$emit('go-to-hex', agent.capital_h3)" title="Ir a la capital">📍</button>
                      <span v-else class="ai-no-capital">—</span>
                      <button class="ai-delete-btn" :disabled="deletingId === agent.player_id"
                        @click="handleDeleteAgent(agent)">{{ deletingId === agent.player_id ? '⏳' : '🗑️' }}</button>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div class="ai-usage-monitor">
              <div class="usage-header">
                ...
                <button class="ctrl-btn btn-usage-reset" :disabled="acting" @click="handleResetStats">↺ Reiniciar</button>
              </div>
              <div class="ai-progress">
                <div class="ai-progress-bar" :style="{ width: budgetPercent + '%' }"
                  :class="{ 'bar-warn': budgetPercent > 50, 'bar-crit': budgetPercent > 80 }"></div>
              </div>
              <div class="usage-numbers">
                {{ (usageTotals.total_tokens || 0).toLocaleString() }} / {{ aiBudget.toLocaleString() }} tokens
                &nbsp;·&nbsp; Coste: ${{ Number(usageTotals.total_cost || 0).toFixed(4) }}
              </div>
              <table v-if="usageRows.length > 0" class="usage-table">
                <thead>
                  <tr>
                    <th>Agente</th><th>Modelo</th><th>Llamadas</th><th>Tokens</th><th>Coste</th><th>Última</th>
                  </tr>
                </thead>
                <tbody>
        ...
        <div class="admin-col col-right">
          ...
          <div class="admin-card">
            ...
            <div class="controls-row" style="margin-top:8px; flex-wrap:wrap">
              <button class="ctrl-btn btn-kafka-test" :disabled="kafkaTesting" @click="handleKafkaTest('tax')">
                {{ kafkaTesting === 'tax' ? '⏳' : '🧪 tax' }}
              </button>
              <button class="ctrl-btn btn-kafka-test" :disabled="kafkaTesting" @click="handleKafkaTest('military')">
                {{ kafkaTesting === 'military' ? '⏳' : '🧪 military' }}
              </button>
              <button class="ctrl-btn btn-kafka-test" :disabled="kafkaTesting" @click="handleKafkaTest('harvest')">
                {{ kafkaTesting === 'harvest' ? '⏳' : '🧪 harvest' }}
              </button>
              <button class="ctrl-btn btn-kafka-test" :disabled="kafkaTesting" @click="handleKafkaTest('production')">
                {{ kafkaTesting === 'production' ? '⏳' : '🧪 producción' }}
              </button>
              <button class="ctrl-btn btn-kafka-test" :disabled="kafkaTesting" @click="handleKafkaTest('salary')">
                {{ kafkaTesting === 'salary' ? '⏳' : '🧪 salary' }}
              </button>
            </div>
            <p class="card-hint" style="margin-top:6px">
              ⚠️ En Docker usar <code>kafka:9092</code>. Revisa <code>KAFKA_BROKERS</code> en <code>.env</code>.
            </p>
          </div>
          ...
          <div class="admin-card">
            ...
            <div class="audit-control-row">
              <button class="ai-toggle-btn" :class="playerAuditEnabled ? 'toggle-on' : 'toggle-off'"
                :disabled="auditTogglingPlayer" @click="handleTogglePlayerAudit">
                <span class="toggle-dot"></span>
            ...
            <div style="margin-top:10px">
              <div class="card-hint" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
              ...
              <div v-else class="alert-list">
                <div v-for="alert in playerAuditAlerts.filter(a=>!a.reviewed_at)" :key="alert.id"
                  class="alert-row" :style="{ borderLeftColor: severityColor(alert.severity) }">
      ...
      <!-- Feedback toast -->
      <div v-if="message" class="admin-toast" :class="messageType">{{ message }}</div>
...
.hbadge-green  { background: rgba(76,175,80,0.18);  color: #81c784; border: 1px solid rgba(76,175,80,0.35); }
.hbadge-yellow { background: rgba(255,193,7,0.18);  color: #ffc107; border: 1px solid rgba(255,193,7,0.35); }
.hbadge-red    { background: rgba(244,67,54,0.18);  color: #ef5350; border: 1px solid rgba(244,67,54,0.35); }
...
.refresh-label { font-size: 0.7rem; color: #555; }
...
.icon-btn:hover:not(:disabled) { background: rgba(255,255,255,0.1); color: #e8d5b5; }
.icon-btn:disabled { opacity: 0.4; cursor: not-allowed; }
.close-btn { font-size: 0.9rem; font-weight: 700; }
...
.col-left  { overflow-y: auto; max-height: calc(100vh - 56px); }
.col-right { overflow-y: auto; max-height: calc(100vh - 56px); }
...
.proc-name { font-size: 0.82rem; font-weight: 600; color: #e8d5b5; }
.proc-desc { font-size: 0.7rem; color: #a89875; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.uptime-row { margin-top: 8px; font-size: 0.75rem; color: #a89875; }
...
.timing-label { font-size: 0.68rem; color: #a89875; text-transform: uppercase; letter-spacing: 0.4px; }
.timing-value { font-size: 0.9rem; font-weight: 700; color: #e8d5b5; font-family: monospace; }
.next-turn-soon { color: #ffc107; }

.timing-edit-row { display: flex; align-items: center; gap: 3px; }
.timing-input {
  width: 56px; background: rgba(0,0,0,0.4);
  border: 1px solid rgba(197,160,89,0.35); border-radius: 4px;
  color: #e8d5b5; font-size: 0.82rem; font-weight: 700; font-family: monospace;
  padding: 2px 5px; text-align: right;
}
.timing-input:focus { outline: none; border-color: rgba(197,160,89,0.7); }
.timing-input:disabled { opacity: 0.5; }
.timing-input-unit { font-size: 0.75rem; color: #a89875; }
.timing-save-btn {
  background: rgba(76,175,80,0.2); border: 1px solid rgba(76,175,80,0.45);
  border-radius: 4px; color: #81c784; font-size: 0.78rem; font-weight: 700;
...
.btn-green  { background: rgba(76,175,80,0.22);  border-color: rgba(76,175,80,0.5);  color: #81c784; }
.btn-red    { background: rgba(244,67,54,0.22);  border-color: rgba(244,67,54,0.5);  color: #ef5350; }
.btn-orange { background: rgba(255,152,0,0.22);  border-color: rgba(255,152,0,0.5);  color: #ffb74d; }
.btn-blue   { background: rgba(33,150,243,0.22); border-color: rgba(33,150,243,0.5); color: #64b5f6; }
.btn-force  { background: rgba(156,39,176,0.18); border-color: rgba(156,39,176,0.4); color: #ce93d8; }
...
.danger-hint  { color: #888 !important; }

.btn-danger-reset {
  background: rgba(183,28,28,0.15); border-color: rgba(239,83,80,0.4); color: #ef9a9a;
}
.btn-danger-reset:hover:not(:disabled) {
  background: rgba(183,28,28,0.3); border-color: #ef5350; color: #fff;
}
.reset-confirm-box {
  background: rgba(183,28,28,0.1); border: 1px solid rgba(239,83,80,0.3);
  border-radius: 6px; padding: 10px 12px;
}
.reset-confirm-text {
  font-size: 0.76rem; color: #ef9a9a; margin: 0 0 10px; line-height: 1.5;
}
.reset-confirm-text strong { color: #fff; }
.reset-confirm-actions { display: flex; gap: 6px; flex-wrap: wrap; }
.btn-danger-confirm {
  background: rgba(183,28,28,0.6); border-color: #ef5350; color: #fff; font-weight: 700;
}
.btn-danger-confirm:hover:not(:disabled) { background: #c62828; }
.btn-cancel {
  background: transparent; border-color: rgba(255,255,255,0.2); color: #999;
}
.btn-cancel:hover:not(:disabled) { background: rgba(255,255,255,0.06); color: #ccc; }

/* ── AI Agents ───────────────────────────────────────────────────────────── */
.ai-count-badge {
  display: inline-block; margin-left: 6px;
  background: rgba(197,160,89,0.2); color: #c5a059;
  font-size: 0.7rem; font-weight: 700; border-radius: 8px; padding: 1px 7px;
...
.ai-toggle-btn {
  ...
  padding: 4px 10px 4px 6px; border-radius: 20px; border: 1px solid;
  cursor: pointer; font-size: 0.72rem; font-weight: 700;
  transition: all 0.15s; min-width: 90px;
}
.ai-toggle-btn.toggle-on  { background: rgba(56,142,60,0.18);  border-color: rgba(56,142,60,0.5);  color: #81c784; }
.ai-toggle-btn.toggle-off { background: rgba(97,97,97,0.15);   border-color: rgba(97,97,97,0.3);   color: #9e9e9e; }
.toggle-dot {
...
.ai-spawn-select {
  background: rgba(0,0,0,0.4); border: 1px solid rgba(197,160,89,0.35);
  border-radius: 5px; color: #e8d5b5; font-size: 0.78rem; padding: 4px 7px; cursor: pointer;
}
.ai-spawn-select:disabled { opacity: 0.5; }

.ai-budget-input {
  width: 85px; background: rgba(255,255,255,0.06);
  border: 1px solid rgba(255,255,255,0.15); border-radius: 5px;
  color: #e0e0e0; font-size: 0.76rem; padding: 4px 6px;
}

.btn-ai-save {
  padding: 4px 8px; min-width: unset;
  background: rgba(197,160,89,0.12); border-color: rgba(197,160,89,0.3); color: #c5a059;
}
.btn-ai-save:hover:not(:disabled) { background: rgba(197,160,89,0.25); }

.btn-ai-test {
  padding: 4px 10px; min-width: unset;
  background: rgba(100,181,246,0.12); border-color: rgba(100,181,246,0.3); color: #64b5f6;
}
...
.ai-error-body { flex: 1; display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.ai-error-title { font-size: 0.76rem; font-weight: 700; color: #ef5350; }
.ai-error-msg   { font-size: 0.72rem; color: #ef9a9a; word-break: break-word; }
.ai-error-time  { font-size: 0.68rem; color: #888; flex-shrink: 0; align-self: flex-start; white-space: nowrap; }

.ai-spawn-row {
  display: flex; align-items: center; gap: 6px;
  margin-bottom: 10px; flex-wrap: wrap;
}
.ai-spawn-label { font-size: 0.75rem; color: #a89875; flex-shrink: 0; }
.ai-spawn-input {
  width: 44px; background: rgba(0,0,0,0.4);
  border: 1px solid rgba(197,160,89,0.35); border-radius: 5px;
  color: #e8d5b5; font-size: 0.82rem; font-weight: 700; font-family: monospace;
...
.btn-ai-spawn {
  background: rgba(76,175,80,0.2); border-color: rgba(76,175,80,0.45); color: #81c784;
  padding: 5px 10px; min-width: 66px;
}
.btn-ai-cycle {
  background: rgba(33,150,243,0.2); border-color: rgba(33,150,243,0.45); color: #64b5f6;
  padding: 5px 10px; min-width: 82px;
}

.ai-empty { font-size: 0.78rem; color: #555; text-align: center; padding: 12px 0 4px; }
...
.ai-th {
  background: rgba(0,0,0,0.3); color: #c5a059; font-weight: 700;
  text-transform: uppercase; letter-spacing: 0.4px;
  padding: 6px 10px; text-align: left;
  border-bottom: 1px solid rgba(197,160,89,0.18); white-space: nowrap;
}
.ai-th-num { text-align: right; }
.ai-th-act { width: 76px; }
.ai-row { border-bottom: 1px solid rgba(197,160,89,0.07); }
.ai-row:last-child { border-bottom: none; }
.ai-row:hover { background: rgba(197,160,89,0.04); }
.ai-td { padding: 6px 10px; color: #e8d5b5; vertical-align: middle; }
.ai-td-name {
  display: flex; align-items: center; gap: 6px;
...
.ai-profile-badge {
  font-size: 0.68rem; background: rgba(197,160,89,0.15); color: #c5a059;
  border-radius: 7px; padding: 2px 6px; white-space: nowrap;
}
.ai-profile-badge.badge-expansionist { background: rgba(178,34,34,0.18); color: #ef9a9a; }
.ai-profile-badge.badge-balanced     { background: rgba(21,101,192,0.18); color: #90caf9; }

.ai-goto-btn {
  background: rgba(197,160,89,0.12); border: 1px solid rgba(197,160,89,0.28);
  color: #c5a059; font-size: 0.72rem; font-weight: 700;
  border-radius: 4px; padding: 2px 6px; cursor: pointer;
}
.ai-goto-btn:hover { background: rgba(197,160,89,0.25); }

.ai-delete-btn {
  background: rgba(244,67,54,0.1); border: 1px solid rgba(244,67,54,0.28);
  color: #ef5350; font-size: 0.72rem; border-radius: 4px; padding: 2px 5px;
  cursor: pointer; margin-left: 3px;
}
.ai-delete-btn:hover:not(:disabled) { background: rgba(244,67,54,0.25); }
.ai-delete-btn:disabled { opacity: 0.4; cursor: not-allowed; }
.ai-no-capital { color: #444; font-size: 0.76rem; }
...
.usage-title { font-size: 0.75rem; font-weight: 700; color: #aaa; letter-spacing: 0.05em; }
.btn-usage-reset {
  font-size: 0.68rem; padding: 2px 7px;
  background: rgba(97,97,97,0.15); border-color: rgba(97,97,97,0.3); color: #9e9e9e;
}
.btn-usage-reset:hover:not(:disabled) { background: rgba(97,97,97,0.3); }
.ai-progress {
  height: 5px; background: rgba(255,255,255,0.07); border-radius: 3px; overflow: hidden; margin-bottom: 4px;
}
.ai-progress-bar {
  height: 100%; border-radius: 3px; background: #42a5f5;
  transition: width 0.4s, background 0.4s; min-width: 2px;
}
.ai-progress-bar.bar-warn { background: #ffa726; }
.ai-progress-bar.bar-crit { background: #ef5350; }
.usage-numbers { font-size: 0.7rem; color: #777; margin-bottom: 6px; }
.usage-table { width: 100%; border-collapse: collapse; font-size: 0.7rem; }
.usage-table th {
  color: #666; font-weight: 600; text-align: left;
  padding: 3px 6px; border-bottom: 1px solid rgba(255,255,255,0.06);
}
.usage-table td {
  color: #ccc; padding: 3px 6px; border-bottom: 1px solid rgba(255,255,255,0.03);
}
.usage-table code {
  font-size: 0.66rem; background: rgba(255,255,255,0.07);
  border-radius: 3px; padding: 1px 3px; color: #90caf9;
}

/* ── Kafka / Audit ───────────────────────────────────────────────────────── */
.kafka-status-row { display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 4px; }
.kafka-meta { font-size: 0.7rem; color: #a09070; }
.kafka-meta code {
  background: rgba(255,255,255,0.06); padding: 1px 4px; border-radius: 3px; font-size: 0.68rem;
}
.btn-kafka-test {
  background: rgba(33,150,243,0.14); border-color: rgba(33,150,243,0.45); color: #64b5f6;
  flex: 0 1 auto;
}
.btn-kafka-test:hover:not(:disabled) { background: rgba(33,150,243,0.28); }

.audit-control-row { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
.audit-stats-row   { display: flex; align-items: center; gap: 8px; }
.audit-stats-block {
...
.alert-row {
  background: rgba(255,255,255,0.03); border-left: 3px solid #f87171;
  border-radius: 0 5px 5px 0; padding: 7px 9px; position: relative;
}
.alert-header { display: flex; align-items: center; gap: 7px; flex-wrap: wrap; margin-bottom: 3px; }
.alert-severity  { font-weight: 700; font-size: 0.68rem; text-transform: uppercase; }
.alert-rule      { font-weight: 600; font-size: 0.75rem; color: #e8d5b5; }
.alert-player    { font-size: 0.75rem; color: #c5a059; }
.alert-time      { font-size: 0.68rem; color: #64748b; margin-left: auto; }
.alert-detail    {
...
.alert-review-btn {
  ...
  color: #86efac; border-radius: 4px; padding: 2px 7px;
  font-size: 0.68rem; cursor: pointer;
}
.alert-review-btn:hover { background: rgba(134,239,172,0.2); }
...
.msg-ok  { background: rgba(56,142,60,0.92);  color: #e8ffe8; border: 1px solid rgba(76,175,80,0.5); }
.msg-err { background: rgba(183,28,28,0.92);  color: #ffe8e8; border: 1px solid rgba(244,67,54,0.5); }

.admin-loading {
  padding: 40px 20px; text-align: center; color: #a89875; flex: 1;
}
.admin-error {
  padding: 16px 20px; text-align: center; color: #ef5350;
  background: rgba(244,67,54,0.08); border-bottom: 1px solid rgba(244,67,54,0.2);
}

/* ── Scrollbar ───────────────────────────────────────────────────────────── */
.admin-body::-webkit-scrollbar { width: 6px; }
.admin-body::-webkit-scrollbar-track { background: rgba(0,0,0,0.2); }
.admin-body::-webkit-scrollbar-thumb { background: rgba(197,160,89,0.3); border-radius: 3px; }
.admin-body::-webkit-scrollbar-thumb:hover { background: rgba(197,160,89,0.5); }
</style>
